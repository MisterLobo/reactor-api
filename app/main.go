...
func basicAuth(next http.HandlerFunc) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if ok {
			...
			realUserHash := sha256.Sum256([]byte(common.AUTH_USER))
			realPassHash := sha256.Sum256([]byte(common.AUTH_PASS))
...
func extractEmbed() {
	...
	if os.IsNotExist(err) {
		os.Mkdir(tmpname, os.ModePerm)
	}
	extract, err := os.Create(path.Join(tmpname, "lsx.tar.gz"))
	defer extract.Close()
	extract.Write(lsx)
}
...
func main() {
	...
	authorized := r.Group("/")
	authorized.Use(gin.WrapF(func(w http.ResponseWriter, r *http.Request) {
		secret := r.Header.Get("X-Secret")
		secretHash := sha256.Sum256([]byte(secret))
		realSecretHash := sha256.Sum256([]byte(common.AUTH_SECRET))
	...
	authorized.Use(gin.BasicAuth(gin.Accounts{
		common.AUTH_USER: common.AUTH_PASS,
	}))
	{
		authorized.
		...
		authorized.
			POST("/containers/create", func(ctx *gin.Context) {
			...
			POST("/containers/prune", func(ctx *gin.Context) {
				r, e := app.ContainersPrune()
				if e != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": e.Error()})
					return
				}
				ctx.JSON(http.StatusOK, gin.H{"report": r})
			}).
			POST("/containers/run", func(ctx *gin.Context) {
				...
				fmt.Println("[run#params]: ", body.Binds, body.Env, body.PortBindings)
			...
			GET("/container/:id", func(ctx *gin.Context) {
				...
				c := app.ContainerGet(&types.ContainerGetParams{ID: params.ID})
				if c == nil {
			...
			POST("/container/:id/put_archive", func(ctx *gin.Context) {
				...
				var body types.ContainerPutArchiveBody
				...
				tmpdir, err := os.MkdirTemp(tmpname, "import")
				if err != nil {
				...
				if err != nil {
					...
					ctx.JSON(http.StatusInternalServerError, gin.H{"error": "could not write to file"})
					return
				}
				...
				go func() {
					...
					if err != nil {
						log.Println("error reading archive:", err.Error())
						che <- err
						return
					}
					...
					defer unc.Close()
					if savePath != "" {
						err = app.ContainerPutArchive(&params, &body, savePath)
						if err != nil {
							log.Println("error parsing archive:", err.Error())
							che <- err
							done <- true
							return
						}
				...
				if err != nil {
					log.Println("error processing archive:", err.Error())
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
					return
				}
				fmt.Println("[ok]: ", ok)
			...
			GET("/container/:id/get_archive", func(ctx *gin.Context) {
				...
				var body types.ContainerGetArchiveBody
				err = ctx.BindJSON(&body)
				if err != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
					return
				...
				chb := make(chan []byte)
				go func() {
					bytes, _, err := app.ContainerGetArchive(&params, &body)
					if err != nil {
						ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
						return
					}
			...
			POST("/container/:id/ls", func(ctx *gin.Context) {
				var params types.ContainerLsParams
				err := ctx.ShouldBindUri(&params)
				if err != nil {
					fmt.Println("cannot bind params:", err.Error())
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
				...
				var body types.ContainerLsBody
				err = ctx.BindJSON(&body)
				if err != nil {
					fmt.Println("cannot bind body:", err.Error())
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
				...
				fmt.Println("[exec#params]:", params, body)
				output, dirs, files, err := app.ContainerLs(&params, &body)
				if err != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
					return
				}
				log.Println("[output]:", len(output))
				ctx.JSON(http.StatusOK, gin.H{"items": output, "dirs": dirs, "files": files})
			}).
			PUT("/container/:id/stopls", func(ctx *gin.Context) {
				var params types.ContainerLsParams
				err := ctx.ShouldBindUri(&params)
				if err != nil {
					fmt.Println("cannot bind params:", err.Error())
				...
				err = app.ContainerLsStop(&params)
				if err != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
					return
				}
			...
			PATCH("/container/:id/updatels", func(ctx *gin.Context) {
				var params types.CommonRequestParams
				err := ctx.ShouldBindUri(&params)
				if err != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
				...
				app.UpdateLsBin(&params)
				ctx.Status(http.StatusOK)
			}).
			GET("/container/:id/hostinfo", func(ctx *gin.Context) {
				var params types.ContainerRequestParams
				err := ctx.ShouldBindUri(&params)
				if err != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
				...
				var query types.ContainerHostInfoQueryParams
				err = ctx.ShouldBind(&query)
				if err != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
					return
				}
				res, err := app.ContainerHostInfo(&params, &query)
				if err != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
					return
				}
				ctx.JSON(http.StatusOK, gin.H{"host": res})
			}).
			...
			POST("/images/build", func(ctx *gin.Context) {
				...
				tmpdir, err := os.MkdirTemp(tmpname, "build")
				if err != nil {
					fmt.Println("error processing file:", err.Error())
					ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
					return
			...
			POST("/images/prune", func(ctx *gin.Context) {
				r, e := app.ImagesPrune()
				if e != nil {
					ctx.JSON(http.StatusBadRequest, gin.H{"error": e.Error()})
					return
		...
		authorized.GET("/volume/:id/inspect", func(ctx *gin.Context) {
